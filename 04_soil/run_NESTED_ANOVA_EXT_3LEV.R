...
fit_ANOVA <- function(dataset) {
  check.mod <- lm(stat ~ group1 / group2 / group3, dataset)
  ...
  hom.group3 <- bartlett.test(residuals(check.mod, type = "pearson") ~ dataset$group3)
  print(paste0("Bartlett p GROUP1: ",hom.group1$p.value))
  print(paste0("Bartlett p GROUP2: ",hom.group2$p.value))
  print(paste0("Bartlett p GROUP3: ",hom.group3$p.value))
  # Getting results
  ...
  out.mod <- data.frame("shap" = c(shap$p.value),
                        ...
                        "Df_GR3" = df.mod["group1:group2:group3","Df"],
                        "F_GR3" = df.mod["group1:group2:group3","F value"],
                        "R2_GR3" = df.mod["group1:group2:group3","Variance"],
                        "p_GR3" = df.mod["group1:group2:group3","Pr(>F)"])
  
  
  # Tukey
  reg.aov <- aov(stat ~ group1 / group2 / group3, data=dataset)
...
fit_GLS <- function(dataframe) {
  mod.gls <- gls(stat ~ group1,
                 correlation = corCompSymm(form = ~ 1 | group1/group2/group3),
  ...
  mod.gls.lev2 <- gls(stat ~ group2,
                      ...
                      dataframe)
  mod.gls.lev3 <- gls(stat ~ group3,
                      correlation = corCompSymm(form = ~ 1 | group1/group2/group3),
                      weights = varIdent(form = ~ 1|group3),
                      dataframe)
  ...
  df.mod.gls.3 <- as.data.frame(anova(mod.gls.lev3))
  ...
  out.mod <- data.frame("Df_GR1" = df.mod.gls["group1","numDF"],
                        ...
                        "Df_GR3" = df.mod.gls.3["group3","numDF"],
                        "F_GR3" = df.mod.gls.3["group3","F-value"],
                        "R2_GR3" = NA,
                        "p_GR3" = df.mod.gls.3["group3","p-value"])
...
remove_outliers <- function(dataset) {
  mod <- lm(stat ~ group1 / group2 / group3, dataset)
  newdf <- dataset
...
#########################################
run_test <- function(dataset, metric_value, group_1, group_2, group_3) {
...
run_nested_models <- function(dataset, metric_value, group_1, group_2, group_3) {
  ...
  check.mod <- lm(stat ~ group1 / group2 / group3, data=ds)
  ...
  if (length(small_group) > 0) {
    ...
    # Value transformation
    print("Value transformation")
    check.mod <- lm(stat ~ group1 / group2 / group3, data=ds.2)
