...
meta.alpha <- meta.alpha %>% filter(Sample.type == "roots")
...
write.table(meta.alpha, file="guilds_04_alpha_roots.tsv", sep='\t', row.names = FALSE, col.names = TRUE, quote = FALSE, append=FALSE)
...
write.table(model.1.out, file = "guilds_04_alpha_roots_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
write.table(df.models, file = "guilds_04_chao1_roots_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
