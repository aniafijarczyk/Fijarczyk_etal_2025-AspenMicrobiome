...
dm <- read.csv("nitro_07_combine_inputs.tsv", sep="\t", header=T)
dt <- read.csv("nitro_07_combine_tests.tsv", sep="\t", header=T)
...
head(pvals)
pvals_spread <- pvals %>% dplyr::select(statistic, Sample.type, metric, adj.p) %>% spread(key=statistic,adj.p) %>% dplyr::rename(adj_GR1 = p_GR1, adj_GR2 = p_GR2)
pvals_spread
...
p1C <- ggplot(dtt) +
  ...
  geom_text(data=dtt, aes(x = 0.5, y = offset+1, label = ifelse(MOD %in% c("ANOVA","ANOVA_transformed"),
                                                                        paste0("Group: R2=",round(R2_GR1,digits=3)," ",sign_GR1,
                                                                               "\nSite: R2=",round(R2_GR2,digits=3)," ",sign_GR2),
...
png('nitro_08_plot_main.png', w=1800, h=2000, res=300)
p1C
dev.off()
