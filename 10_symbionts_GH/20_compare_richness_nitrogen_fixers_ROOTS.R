...
head(dmeta)
dmeta.soil <- dmeta %>% dplyr::filter(Sample.type == "roots") %>% dplyr::filter(Cluster != "neg control extraction")
dim(dmeta.soil)
head(dmeta.soil)
...
write.table(dalpha, file="nitro_06_chao_roots.tsv", sep='\t', row.names = FALSE, col.names = TRUE, quote = FALSE, append=FALSE)
...
write.table(df.models, file = "nitro_06_chao_roots_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
p1C <- ggplot(dalpha) +
  geom_boxplot(data=dalpha, aes(x = Cluster, y = chao1, fill = Population), outlier.shape = NA) +
  geom_point(data=dalpha, aes(x = Cluster, y = chao1), color="grey40",
             pch=21, size=6, position=position_jitter(width=0.1)) +
  
  annotate("text", x = 0.5, y = 26, 
...
#png('nitro_06_chao_roots.png', w=800, h=1200, res=300)
#p1C
#dev.off()
