...
meta.alpha <- meta.alpha %>% filter(Sample.type == "rhizosphere")
...
write.table(meta.alpha, file="guilds_04_alpha_rhizo.tsv", sep='\t', row.names = FALSE, col.names = TRUE, quote = FALSE, append=FALSE)
...
meta.alpha$stat <- meta.alpha$log_gini_trans
meta.alpha$group1 <- meta.alpha$Population
meta.alpha$group2 <- meta.alpha$Cluster
model.1.out <- fit_ANOVA(meta.alpha)
model.1.out$MOD <- "ANOVA"
model.1.out


write.table(model.1.out, file = "guilds_04_alpha_rhizo_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
write.table(df.models, file = "guilds_04_chao1_rhizo_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
