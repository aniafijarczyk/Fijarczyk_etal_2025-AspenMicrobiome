...
write.table(dalpha, file="nitro_05_alpha_rhizo.tsv", sep='\t', row.names = FALSE, col.names = TRUE, quote = FALSE, append=FALSE)
...
write.table(df.models, file = "nitro_05_alpha_rhizo_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
p1C <- ggplot(dalpha) +
  ...
  #ggtitle("Population") + 
  scale_y_continuous(limits=c(-0.85,0.15)) +
...
#png('nitro_05_alpha_rhizo.png', w=800, h=1200, res=300)
#p1C
#dev.off()
