...
df <- read.csv("./guilds_02_read_greenhouse_rhizo.csv", header=TRUE, sep=',')
head(df)
df.meta <- read.csv("./guilds_02_read_greenhouse_meta_rhizo.csv", header=TRUE, sep=',')
head(df.meta)
...
pseq.16s.soil <- subset_samples(pseq.stands, sample_data(pseq.stands)$Sample.type=="rhizosphere")
pseq.16s.soil
...
head(dsub.stands)
write.table(dsub.stands, file = "guilds_03_clr_rhizo_inputs.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
write.table(dOUT, file = "guilds_03_clr_rhizo_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
