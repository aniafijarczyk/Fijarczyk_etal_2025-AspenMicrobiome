...
head(dmeta)
dmeta.soil <- dmeta %>% dplyr::filter(Sample.type == "rhizosphere") %>% dplyr::filter(Cluster != "neg control extraction")
dim(dmeta.soil)
head(dmeta.soil)
...
write.table(dalpha, file="nitro_06_chao_rhizo.tsv", sep='\t', row.names = FALSE, col.names = TRUE, quote = FALSE, append=FALSE)
...
write.table(df.models, file = "nitro_06_chao_rhizo_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
#png('nitro_06_chao_rhizo.png', w=800, h=1200, res=300)
#p1C
#dev.off()
