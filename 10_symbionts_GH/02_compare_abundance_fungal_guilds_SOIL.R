...
df <- read.csv("./guilds_02_read_greenhouse.csv", header=TRUE, sep=',')
head(df)
df.meta <- read.csv("./guilds_02_read_greenhouse_meta.csv", header=TRUE, sep=',')
head(df.meta)
...
head(dsub.stands)
write.table(dsub.stands, file = "guilds_03_clr_soil_inputs.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
...
write.table(dOUT, file = "guilds_03_clr_soil_tests.tsv", sep="\t", row.names = F, col.names = T, append=F, quote=F)
