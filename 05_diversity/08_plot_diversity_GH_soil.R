...
dm <- dm_ %>% filter(Population %in% c("NENA","MX") & Sample.type %in% "bulk soil") %>% 
  gather(key = "metric", value="value", c(chao1, log_gini))
head(dm)

dt_ <- rbind(dt_1, dt_2)
dt <- dt_ %>% filter(EVAL == "OK" & source %in% "soil")
head(dt)
...
png('02_plot_markers_16S.png', w=1800, h=1400, res=300)
p1C
dev.off()
...
png('02_plot_markers_ITS.png', w=1800, h=1400, res=300)
p1A
dev.off()
