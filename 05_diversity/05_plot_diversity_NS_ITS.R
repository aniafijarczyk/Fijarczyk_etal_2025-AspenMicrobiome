...
dm_ <- read.csv("03_alpha_16S_stats.tsv", sep="\t", header=T)
dt1 <- read.csv("04_alpha_ITS_country_tests.tsv", sep="\t", header=T)
dt1_ <- read.csv("04_alpha_ITS_region_tests.tsv", sep="\t", header=T)

dt2 <- read.csv("04_chao1_ITS_country_tests.tsv", sep="\t", header=T)
dt2_ <- read.csv("04_chao1_ITS_region_tests.tsv", sep="\t", header=T)
...
#dm_2 <- dm_ %>% filter(source=="Soil" & marker == "ITS") %>% dplyr::select(long.name, site, Country, region, log_gini, chao1) %>% gather(key = "Group", value = "loc", c("site","region"))
dm_2 <- dm_ %>% filter(source=="Soil" & marker == "ITS") %>% dplyr::select(long.name, site, Country, region, log_gini, chao1) 
...
p1C <- ggplot(dtt1) +
  ...
  geom_line(data=data.frame("x"=c(1.5,3.8),"y"=c(650,650), Metric="Richness", Grouping="site"), aes(x=x, y=y), color="black") +
  geom_line(data=data.frame("x"=c(4.2,7),"y"=c(650,650), Metric="Richness", Grouping="site"), aes(x=x, y=y), color="black") +
  geom_line(data=data.frame("x"=c(1.5,7),"y"=c(750, 750), Metric="Richness", Grouping="site"), aes(x=x, y=y), color="black") +
  
  geom_text(data=data.frame("x"=c(2.65),"y"=c(690),"label"=c("ns."), Metric="Richness", Grouping="site"), aes(x=x,y=y,label=label),size=5) +
  geom_text(data=data.frame("x"=c(5.8),"y"=c(658),"label"=c("*"), Metric="Richness", Grouping="site"), aes(x=x,y=y,label=label),size=8) +
  geom_text(data=data.frame("x"=c(4.2),"y"=c(758), "label"=c("**"), Metric="Richness", Grouping="site"), aes(x=x,y=y,label=label),size=8) +  
...
png('08_plot_main_ITS.png', w=1200, h=1800, res=300)
p1C
dev.off()
