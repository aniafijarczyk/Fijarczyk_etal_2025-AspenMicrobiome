...
dm <- dm_ %>% filter(Population %in% c("NENA","MX") & Sample.type %in% "rhizosphere") %>% 
  gather(key = "metric", value="value", c(chao1, log_gini))
head(dm)

dt_ <- rbind(dt_1, dt_2)
dt <- dt_ %>% filter(EVAL == "OK" & source %in% "rhizo")
head(dt)
...
dg$manual_offset <- c(1202,300, 0.008,0.05)
dg
...
p1C <- ggplot(dm_16) +
  ...
  geom_text(data=dtt1_16, aes(x = 0.5, y = manual_offset, label = ifelse(MOD %in% c("ANOVA","ANOVA_transformed"),
                                                                        paste0("Group: R2=",round(R2_GR1,2)," ",sign_GR1,
                                                                               "\nSite: R2=",round(R2_GR2,3)," ",sign_GR2),
...
png('03_plot_markers_rhizo_16S.png', w=1800, h=1400, res=300)
p1C
dev.off()
...
png('03_plot_markers_rhizo_ITS.png', w=1800, h=1400, res=300)
p1A
dev.off()
