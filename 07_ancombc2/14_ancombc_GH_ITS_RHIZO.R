...
df_asv <- read.csv("../19_ancombc_gh_soil/ancombc_02_input_ITS_ASV.tsv", sep="\t", header=T)
df_asv[1:10, 1:10]
df_asv$genus_taxon

df_tax <- read.csv("../19_ancombc_gh_soil/ancombc_02_input_ITS_TAX.tsv", sep="\t", header=T)
df_tax %>% head()
...
pseq.soil <- subset_samples(pseq, sample_data(pseq)$Sample.type=="rhizosphere")
pseq.soil
...
#head(sample_data(pseq.its.soil.2))
#sample_data(pseq.its.soil.2)$Country
# Changing ASV names
#taxa_names(pseq.its.soil.2) <- paste("ASV-", 1:ntaxa(pseq.its.soil.2), sep="") 
#taxa_names(pseq.its.soil.2)
