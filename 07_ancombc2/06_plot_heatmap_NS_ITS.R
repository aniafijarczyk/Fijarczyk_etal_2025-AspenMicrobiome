...
# log fold changes
res <- read.csv("ancombc_04_run_ITS_results_random_effect.tsv", sep="\t", header=T)
res <- res %>% dplyr::select(-taxon)
dim(res)
head(res)
# Combine with short taxon name 
tax <- read.csv("ancombc_02_input_ITS_TAX.tsv", sep="\t", header=T)
...
# metadata
dmeta <- read.csv("../01_data_prep/dataPrep_01_overview_ITS_META.tsv", sep="\t", header=T)
...
# abundances
abd <- read.csv("ancombc_02_input_ITS_ASV.tsv", sep="\t", header=T)
...
write.table(mg_abd, "ancombc_08_plot_ITS_abundances_with_meta.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
...
head(tres)
tres %>% filter(endsWith(taxon,"Fungi"))
tres %>% filter(endsWith(taxon,"Ascomycota"))

top <- tres %>% filter(!endsWith(taxon,"Fungi")) %>% filter(!endsWith(taxon,"Ascomycota")) %>% arrange(rank) %>% head(30)
top %>% head()
...
decto <- read.csv("ancombc_09_ITS_EMF.tsv", sep="\t", header = T)
decto$Ecto <- "MF"
decto <- decto %>% filter(taxon_org %in% top_ordered$taxon_org)
head(decto)
dim(decto)


####
decto$taxon_org <- factor(decto$taxon_org, levels = top_ordered$taxon_org)
palette_OkabeIto_black <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", 
                            "#0072B2", "#D55E00", "#CC79A7", "#000000")
p4 <- ggplot(decto) + aes(x = Ecto, y = taxon_org) +
  geom_tile(aes(fill = MF), color='white') +
  #coord_fixed() +
  scale_fill_manual(values=c("#004D40","grey","white"), name="Guild") +
  ylab('') +
  xlab(' ') +
  facet_grid(~ Ecto, scales = "free_x", space = "free",
             as.table = TRUE,drop = TRUE, switch="x") +
  theme(panel.background = element_blank(),
        #strip.placement = "outside",
...
spread_asv_lfc_mf <- merge(spread_asv_lfc, decto[,c("taxon_org","MF","Ecto")], by = c("taxon_org"), sort=F)
head(spread_asv_lfc_mf)
...
spread_asv_lfc_mf$taxon <- factor(spread_asv_lfc_mf$taxon, levels = tax_order, labels = vtax)
head(spread_asv_lfc_mf)

#write.table(spread_asv_lfc_mf, file="ancombc_08_plot_ITS_table.tsv", sep="\t", col.names = T,row.names = F, append=F, quote=F)



p1 <- ggplot(spread_asv_lfc_mf) + 
  
  geom_tile(data=spread_asv_lfc_mf, aes(x = 1, y = taxon, fill = AMOS)) +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 2, y = taxon, fill = STFE)) +
  
  geom_tile(data=spread_asv_lfc_mf, aes(x = 4, y = taxon, fill = STET)) +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 5, y = taxon, fill = ESSI)) +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 6, y = taxon, fill = FORE)) +
  
  geom_tile(data=spread_asv_lfc_mf, aes(x = 8, y = taxon, fill = Santiago)) +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 9, y = taxon, fill = FLOR1)) +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 10, y = taxon, fill = FP2)) +
  ...
  new_scale_fill() +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 12, y = taxon, fill = lfc_RegionCold_temperate)) +
  scale_fill_gradient2(high="orchid4", mid="white", low="#1f77b4", name="LFC\nCold temp-\nBoreal",
                       guide = guide_legend(direction="horizontal", order = 2, nrow=1,
                                            theme=theme(legend.text.position = "bottom"))) +
  
  new_scale_fill() +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 13, y = taxon, fill = lfc_RegionWarm_temperate)) +
  scale_fill_gradient2(high="#ff7f0e", mid="white", low="#1f77b4", name="LFC\nWarm temp-\nBoreal",
                       guide = guide_legend(direction="horizontal", order = 3, nrow=1,
                                            theme=theme(legend.text.position = "bottom"))) +
  
  new_scale_fill() +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 14, y = taxon, fill = lfc_RegionWarm_temperate_RegionCold_temperate)) +
  scale_fill_gradient2(high="#ff7f0e", mid="white", low="orchid4", name="LFC\nWarm temp-\nCold temp",
                       guide = guide_legend(direction="horizontal", order = 4, nrow=1,
                                            theme=theme(legend.text.position = "bottom"))) +
  
  new_scale_fill() +
  geom_tile(data=spread_asv_lfc_mf, aes(x = 16, y = taxon, fill = MF)) +
  scale_fill_manual(values=c("#004D40","grey","wheat"), name="Guild",
                    guide = guide_legend(direction="vertical",order = 5,
  ...
  scale_x_discrete(breaks = c(1,2,4,5,6,8,9,10,12,13,14,16), limits = c(1,2,4,5,6,8,9,10,12,13,14,16),
                   labels=c("AMOS","STFE","STET","ESSI","FORE","Santiago","FLOR1","FP2",
                            "CT vs B", "WT vs B", "WT vs CT", "MF"))
  
p1

png('ancombc_08_plot_ITS_heatmap.png', w=2800,h=2400, res=300)
p1
dev.off()
