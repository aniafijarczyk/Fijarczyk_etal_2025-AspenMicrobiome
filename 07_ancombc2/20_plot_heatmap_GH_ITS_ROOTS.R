...
tax <- read.csv("../19_ancombc_gh_soil/ancombc_02_input_ITS_TAX.tsv", sep="\t", header=T)
...
head(dmeta)
samples <- dmeta %>% filter(Sample.type == "roots") %>% filter(Population %in% c("NENA","MX")) %>% pull(long.name)
length(samples)
samples

# abundances
abd <- read.csv("../19_ancombc_gh_soil/ancombc_02_input_ITS_ASV.tsv", sep="\t", header=T)
...
p1 <- ggplot(spread_asv_lfc) + 
  ...
  scale_fill_gradient2(high="#1f77b4", mid="white", low="#ff7f0e", name="LFC\nCanada-\nMexico",
                       guide = guide_legend(direction="horizontal", order = 2, nrow=1),limits=c(-4,6)) +
  coord_fixed() +
