...
# metadata
dmeta <- read.csv("../../2022_MicrobiomeAspenGreenhouse/01_data_prep/dataPrep_01_overview_ITS_META.tsv", sep="\t", header=T)
head(dmeta)
samples <- dmeta %>% filter(Sample.type == "bulk soil") %>% filter(Population %in% c("NENA","MX")) %>% pull(long.name)
length(samples)
samples
...
write.table(mg_abd, "ancombc_06_plot_ITS_abundances_with_meta.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
...
top <- tres %>% filter(!endsWith(taxon,"Fungi")) %>% filter(!endsWith(taxon,"Ascomycota")) %>% arrange(rank) %>% head(20)
top %>% head()
...
top_avs$Site <- factor(top_avs$Cluster, levels = c("AB","FORE","FP","SP"), labels = c("AB","FORE","FP2","Santiago"))
...
p1 <- ggplot(spread_asv_lfc) + 
  ...
  scale_fill_gradient2(high="#1f77b4", mid="white", low="#ff7f0e", name="LFC\nCanada-\nMexico",
                       guide = guide_legend(direction="horizontal", order = 2, nrow=1),limits=c(-3,6)) +
  coord_fixed() +
...
png('ancombc_06_plot_ITS_heatmap.png', w=2900,h=2600, res=300)
p1
dev.off()
