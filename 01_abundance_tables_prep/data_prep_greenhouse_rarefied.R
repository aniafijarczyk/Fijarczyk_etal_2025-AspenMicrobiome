...
otu.file.16s <- "../DATA/PeuplierGH_jan2024/16S/all/PeuplierGH_16S_jan2024.ASV_table_rarefied_7921_dnNA.tsv"
...
tax.matrix.spread %>% head()

TAX = tax_table(tax.matrix.spread)

# Getting sample_data
dmeta.16s <- read.csv("../DATA/PeuplierGH_jan2024/metadata_16S.txt", sep="\t", header=T)
dmeta.16s %>% head()
dmeta.16s$long.name <- gsub('-','.',paste0('X',dmeta.16s$SampleID))
rownames(dmeta.16s) <- dmeta.16s$long.name
head(dmeta.16s)

df.cols <- data.frame('long.name' = colnames(OTU))
dmeta.16s.flt <- merge(df.cols, dmeta.16s, by = "long.name", sort=FALSE)
rownames(dmeta.16s.flt) <- dmeta.16s.flt$long.name

dim(dmeta.16s)
dim(dmeta.16s.flt)
samdata <- sample_data(dmeta.16s.flt)
#samdata <- sample_data(dmeta.16s)
...
otu.file.its <- "../DATA/PeuplierGH_jan2024/ITS/all/PeuplierGH_ITS_jan2024.ASV_table_rarefied_13594_dnNA.tsv"
...
# Getting sample_data
dmeta.its <- read.csv("../DATA/PeuplierGH_jan2024/metadata_ITS.txt", sep="\t", header=T)
dmeta.its %>% head()
dmeta.its$long.name <- gsub('-','.',dmeta.its$SampleID)
rownames(dmeta.its) <- dmeta.its$long.name

df.cols <- data.frame('long.name' = colnames(OTU))
dmeta.its.flt <- merge(df.cols, dmeta.its, by = "long.name", sort=FALSE)
rownames(dmeta.its.flt) <- dmeta.its.flt$long.name


dmeta.its.flt <- dmeta.its %>% filter(long.name %in% colnames(OTU))
dim(dmeta.its)
dim(dmeta.its.flt)
samdata <- sample_data(dmeta.its.flt)
...
save(pseq.16s, pseq.its, pseq.p16s, pseq.pits, file = "AVS_filtered_phyloseq_rarefied.RData")
