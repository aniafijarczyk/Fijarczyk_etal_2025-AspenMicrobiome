...
pseq.sub <- subset_samples(pseq.its, sample_data(pseq.16s)$source=="Soil")
pseq.sub
...
write.table(dm3, file = "05_core_ITS_sample_ASV.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
...
write.table(out3, file = "05_core_ITS_site_ASV.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
...
# Get abundances 
abd <- read.csv("../15_ancombc_region/ancombc_02_input_ITS_ASV.tsv", sep="\t", header=T)
#abd <- as.data.frame(otu_table(pseq.rel))
abd[1:10, 1:10]
dim(abd)
...
#head(abd.rel)
prevs <- rowSums(abd > 0.001) %>% as.data.frame()
colnames(prevs) <- "count"
...
write.table(dm3, file = "02_core_ITS_sample_GENUS.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
...
write.table(out2, file = "05_core_ITS_site_GENUS.tsv", sep="\t", row.names=F, col.names=T, quote=F, append=F)
